...
async def _download_resume(resume_url: str) -> tuple[bytes, str, Optional[str]]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            supported_types = ["application/pdf", "application/octet-stream", "officedocument", "msword"]
...
async def _convert_doc_to_pdf_if_needed(file_content: bytes, filename: str) -> tuple[bytes, str]:
...
async def init_file(file, resume_url):
    ...
    if file:
          ...
          file_content, filename, file_id = await _download_resume(resume_url)
